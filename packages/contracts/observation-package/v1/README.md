...
Each bounding-box trace point includes timestamp, normalized bounding box, and optional confidence. The normalized box carries target position and size in the camera frame without classifying the object.
...
- `devicePoseTrace`
- optional `gyroscopeTrace`
- optional `accelerometerTrace`
...
Optional audio features can include level summaries, coarse band energy, signal-to-noise, wind proxy, and noise proxy values. Packages remain valid without audio evidence.
...
- `featureFlags`
- `mlReadiness`
- optional `compactFeatureSummary`

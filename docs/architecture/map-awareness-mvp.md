...
## First iOS Prototype Layer

The first iOS implementation is local-first. It does not require backend map-feed integration yet.

Source:

- latest completed local Observation Package preview generated by the capture flow
- future replacement/augmentation source: backend `GET /map-feed`

Rendered fields:

- marker location from `validationJoin.observerLocation.lat`
- marker location from `validationJoin.observerLocation.lon`
- location accuracy from `validationJoin.observerLocation.accuracyMeters`
- spatial uncertainty from `validationJoin.spatialUncertaintyMeters`
- preferred heading from `validationJoin.roughBearingDegrees`
- fallback heading from `evidence.motion.bearingEstimate.degrees`
- quality label from `derivedEvidence.qualityTier`
- observed time from `createdAt`
- duration from `evidence.tracking.durationMs`
- readable reasons from `derivedEvidence.reasonCodes` and `derivedEvidence.insufficiencyReasons`

Missing-data behavior:

- no completed local package: show "No observations yet"
- completed package without location: show "Latest observation has no location"
- completed package without heading: render marker only and show "Direction unavailable"

The first visual layer uses:

- one marker for the latest completed observation
- an accuracy/uncertainty ring when uncertainty is available
- a short direction ray when heading is available
- a compact marker detail card after marker selection

The implementation intentionally avoids:

- raw sensor trace display
- clustering
- backend dependency
- classification or object identity claims
- route prediction
